...
const states = Object.values(PRODUCT_GENDER).map((state) =>
  state.replace('_', ' '),
)

export const GenderFilter = () => {
  const [open, setOpen] = useState(false)
  const value = useProductsFilters((state) => state.filters.gender)
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className='w-[290px] p-0'>
        <Command>
          <CommandInput placeholder='Selecciona un género...' className='h-9' />
          ...
          <CommandGroup className='overflow-y-auto max-h-[300px]'>
            {states.map((state) => (
              <CommandItem
                ...
                onSelect={() => {
                  if (state === value) {
                    setValue({ key: 'gender', value: undefined })
                  } else {
                    setValue({
                      key: 'gender',
                      value: state,
